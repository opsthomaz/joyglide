...
| `button_map` | dict | see `DEFAULT_BUTTON_MAP` | Button → action (`left` / `right` / `middle` / `back` / `forward` / `none`) for L, R, ZL, ZR, STICK, A, Y, D-pad ←/→. Edited in the Buttons tab (changes go live on Apply; warns if a side is left with no click); invalid entries reset per button on load |
